...
fn parse_num(input: &[u8]) -> i16 {
    ...
    let (d1, d2, d3) = match (negative, len) {
        ...
        _ => {println!("{:?}", str::from_utf8(input)); panic!()},
    };
    ...
    'main: loop {
        match f.read(&mut buff) {
            Ok(ref n) if *n == BUF_SIZE => {
                let first_n = buff.first_e(b'\r');
                let last_n = buff.last_e(b'\r');
                ...
                let main_buffer = buff[first_n..last_n].as_mut();

                city_map.decode_slice(&overflow_slice[1..overflow_slice.len()]);
                
                counter += 1;
                for l in main_buffer.split_mut(|b| *b == b'\r') {
                    if l.len() == 0{continue;}
                    //println!("{:?}", str::from_utf8(l));
            ...
            Ok(n) => {
                ...
                let semi = overflow_buffer.first_e(0);
                let overflow_slice = &overflow_buffer[0..semi];
                let main_buffer = buff[first_n..n].as_mut();
